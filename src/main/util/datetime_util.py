...
def from_iso_totimestamp(dt, use_tz=None):
...
def time_from(datetime):
    """Calculate time from UNIX timestamp till now."""
    now = datetime.now()
    if datetime > now:
        return timespan(0, 0, 0, 0, 0, 0, 0, 0, 0)
    return get_timespan(now - datetime)
...
def get_formatted_gmt_datetime_string():
    """Returns a formated gmt datetime string"""
    gmt_time = time.gmtime()
    gmt_time_to_dt = datetime.datetime.fromtimestamp(
        mktime(gmt_time), tz=pytz.timezone("GMT")
    )
    return gmt_time_to_dt.strftime("%Y-%m-%d %H:%m:%s")
